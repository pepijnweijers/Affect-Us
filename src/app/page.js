...
function AnswerOption({ answer, answerIndex, questionIndex, handleKeyDown, updateStep }) {
    return (
        <div key={answerIndex} className="flex-1">
            <label
                ...
                tabIndex={0}
                onKeyDown={(e) => handleKeyDown(e, `answer-${questionIndex}-${answerIndex}`)}
                htmlFor={`answer-${questionIndex}-${answerIndex}`}
            >
                <div
                    className="w-full h-full flex items-center justify-center opacity-50 group-focus:opacity-100 motion-reduce:duration-0 duration-500"
                    style={{ backgroundColor: answer.color }}
                >
                    {answer.icon && <IconMoodHappyFilled size={192} />}
                </div>
                <input
            ...
            <div
                className="absolute motion-reduce:!opacity-0 peer-focus:animate-fadeHighlight opacity-0 inset-0 -z-10 duration-300"
                style={{ backgroundColor: answer.color }}
            />
        </div>
    );
...
function Question({ question, index, totalQuestions, handleKeyDown, updateStep }) {
    return (
        <div className="flex flex-col py-10 min-h-screen">
            <div>
                <p tabIndex={0} className="font-medium">
                    Vraag {index + 1} van de {totalQuestions}
                </p>
                <h2
                    id={`question-${index}`}
...
export default function Home() {
    ...
    useEffect(() => {
        ...
        if (questionElement) questionElement.focus();
    }, [step]);
